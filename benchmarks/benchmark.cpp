...
int sc_main(int argc, char** argv)
{
    ...
    using a_ns = std::chrono::duration<double, std::nano>;
    using ns = std::chrono::duration<double, std::nano>;
    // for milliseconds, use using ms = std::chrono::duration<double, std::milli>;
    const auto before = clock::now();
    ns duration_global;
    duration_global= clock::now() - before;
    std::cout <<" Before: " << duration_global.count() << "ns" << std::endl;
    if(argc !=  6){
    ...
    //ofstream outFile;
    //outFile.open("../traces/trace-end.txt");
    ...
    // ------------------- XILINX_MAO ---------------------
    std::unique_ptr<XilinxMAO> MAO = std::make_unique<XilinxMAO>("XilinxMAO",
    ...
    for(unsigned i = 0; i < BMs; i++){
        ...
        if(benchmark == "SCS"){
            ...
            CPUs.emplace_back(
                ...
                //std::cout << "Hi \n" ; //each of these are done 32 times
            CPUs.emplace_back(
                std::make_unique<RW_RandomCPU>(
                    name, CPUbuswidthInByte, CPUfreqInMHz, *term,
                    START_ADDRESS, STRIDE, BURST_LENGTH, WIDTH, REP,
